...
-  **Environments Protection Rules:** - Create an environment protection rule named **approve-after-readiness-check**, which will be used to approve the workflow before it proceeds to the next stage following the readiness check.

- **To create:** *Repo Settings → Environments → Select / Create Environment (approve-after-readiness-check) → Configure Protection Rules → Enable Required Reviewers → Add Users/Teams → Save configuration*
...
- A sample file named gitlab-stats-sample.csv is available in the root directory of the repository for reference.
...
- Provide the required input parameters. (inventory file name, GitHub Type)
- Click Run workflow to start the migration process.

### 5.1 Workflow Stages 

- Validate workflow inputs, configuration, and prerequisites
- Run readiness check to check active merge requests and running pipelines
- Pause for manual approval (review readiness output before proceeding)
- Generate GitLab repository migration archives
- Upload migration archives to intermediate storage
- Initiate GitHub repository migrations using uploaded archives
- Display final migration summary (successes, failures, and migration IDs)
- Preserve logs, reports, and outputs as pipeline artifacts
